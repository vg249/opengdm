...
public class JobStatusReporter {
    ...
    public String getExtractorInstructionFileFqpn(String cropType) throws Exception {
        ...
        String returnVal = configSettings.getCropProcessingPath(cropType, GobiiFileProcessDir.EXTRACTOR_INSTRUCTIONS)
                + this.jobId
                + this.fileExtension;

        if (!new File(returnVal).exists()) {

            returnVal = configSettings.getCropProcessingPath(cropType, GobiiFileProcessDir.EXTRACTOR_INPROGRESS)
                    + this.jobId
                    + this.fileExtension;

            if (!new File(returnVal).exists()) {

                returnVal = configSettings.getCropProcessingPath(cropType, GobiiFileProcessDir.EXTRACTOR_INSTRUCTIONS)
                        + this.jobId
                        + this.fileExtension;

                if (!new File(returnVal).exists()) {
                    returnVal = configSettings.getCropProcessingPath(cropType, GobiiFileProcessDir.EXTRACTOR_DONE)
                            + this.jobId
                            + this.fileExtension;
                }
            }
    ...
    public String getLoaderInstructionFileFqpn(String cropType) throws Exception {
        ...
        String returnVal = configSettings.getCropProcessingPath(cropType, GobiiFileProcessDir.LOADER_INSTRUCTIONS)
                + this.jobId
                + this.fileExtension;

        if (!new File(returnVal).exists()) {

            returnVal = configSettings.getCropProcessingPath(cropType, GobiiFileProcessDir.LOADER_INPROGRESS_FILES)
                    + this.jobId
                    + this.fileExtension;

            if (!new File(returnVal).exists()) {
                returnVal = configSettings.getCropProcessingPath(cropType, GobiiFileProcessDir.LOADER_INTERMEDIATE_FILES)
                        + this.jobId
                        + this.fileExtension;

                if (!new File(returnVal).exists()) {
                    returnVal = configSettings.getCropProcessingPath(cropType, GobiiFileProcessDir.LOADER_DONE)
                            + this.jobId
                            + this.fileExtension;
