...
*/		
		...
		HelperFunctions.tryExec("rm "+tempDir+"tmp");
		
		HelperFunctions.tryExec("cat "+tempDir+"map.header "+tempDir+"map.body",outFile,errorFile);
		HelperFunctions.tryExec("rm "+tempDir+"map.header");
		HelperFunctions.tryExec("rm "+tempDir+"map.body");
		return true;
	}
...
7) add header file to top
		...
		HelperFunctions.tryExec("cut -f1 "+markerFile,tempDir+"tmp",errorFile);
		HelperFunctions.tryExec("tail -n +2 "+tempDir+"tmp",tempDir+"genotype.markerList",errorFile);
		HelperFunctions.tryExec("rm "+tempDir+"tmp");
		//Adds one line, then rotates 90 degrees, so each marker is a tab
		HelperFunctions.tryExec("cat "+tempDir+"blank.file "+tempDir+"genotype.markerList",tempDir+"tmp",errorFile);
		HelperFunctions.tryExec("tr '\\n' '\\t'",tempDir+"genotype.markerIList",errorFile,tempDir+"tmp");//Input redirection wheeee
		HelperFunctions.tryExec("rm "+tempDir+"tmp");
		//Note, this file has no line ending.
		
		//Sample list is all samples, no header
		HelperFunctions.tryExec("cut -f1 "+sampleFile,tempDir+"tmp",errorFile);
		HelperFunctions.tryExec("tail -n +2 "+tempDir+"tmp",tempDir+"genotype.sampleList",errorFile);
		HelperFunctions.tryExec("rm "+tempDir+"tmp");
		...
		HelperFunctions.tryExec("cat "+tempDir+"map.header "+ tempDir+"genotype.markerIList " + tempDir+"blank.file "+ tempDir+"sample.matrix",outFile,errorFile);
		
		HelperFunctions.tryExec("rm "+tempDir+"map.header");
		HelperFunctions.tryExec("rm "+tempDir+"genotype.markerList");
		HelperFunctions.tryExec("rm "+tempDir+"genotype.markerIList");
		HelperFunctions.tryExec("rm "+tempDir+"genotype.sampleList");
		HelperFunctions.tryExec("rm "+tempDir+"sample.matrix");
		HelperFunctions.tryExec("rm "+tempDir+"blank.file");
