...
public class ConfigValuesProps implements ConfigValues {
    ...
    public ConfigValuesProps() throws Exception {
        this(null);
    }

    public ConfigValuesProps(String fqpn) throws Exception {

        this.fileReaderProps = new ConfigFileReaderProps(fqpn);

        String currentPrefix = null;

        String candidateCropName = fileReaderProps.getPropValue(PROP_NAME_WEB_SVR_DEFAULT_CROP).toUpperCase();
        ...
        emailSvrType = fileReaderProps.getPropValue(PROP_NAME_MAIL_SVR_TYPE);
        emailSvrDomain = fileReaderProps.getPropValue(PROP_NAME_MAIL_SVR_DOMAIN);

        if (null != fileReaderProps.getPropValue(PROP_NAME_MAIL_SVR_PORT) && NumberUtils.isNumber(fileReaderProps.getPropValue(PROP_NAME_MAIL_SVR_PORT))) {
            emailServerPort = Integer.parseInt(fileReaderProps.getPropValue(PROP_NAME_MAIL_SVR_PORT));
        }

        emailSvrUser = fileReaderProps.getPropValue(PROP_NAME_MAIL_SVR_USER);
        emailSvrHashType = fileReaderProps.getPropValue(PROP_NAME_MAIL_SVR_HASHTYPE);
        emailSvrPassword = fileReaderProps.getPropValue(PROP_NAME_MAIL_SVR_PWD);
        iflIntegrityCheck = fileReaderProps.getPropValue(PROP_NAME_IFL_INTEGRITY_CHECK).equals("true");

        fileSystemRoot = fileReaderProps.getPropValue(PROP_NAME_FILE_SYSTEM_ROOT);
        ...
        for (int idx = 0; idx < cropPrefixes.length; idx++) {
            ...
            String serviceDomain = fileReaderProps.getPropValue(currentPrefix + CROP_SUFFIX_SERVICE_DOMAIN);
            String serviceAppRoot = fileReaderProps.getPropValue(currentPrefix + CROP_SUFFIX_SERVICE_APPROOT);
            Integer servicePort = Integer.parseInt(fileReaderProps.getPropValue(currentPrefix + CROP_SUFFIX_SERVICE_PORT));
            String userFilesLocation = fileReaderProps.getPropValue(currentPrefix + CROP_SUFFIX_USER_FILE_LOCLOCATION);
            String loaderFilesLocation = fileReaderProps.getPropValue(currentPrefix + CROP_SUFFIX_LOADR_FILE_LOCATION);
            String extractorFilesLocation = fileReaderProps.getPropValue(currentPrefix + CROP_SUFFIX_EXTRACTOR_FILE_LOCATION);
            String extractorFilesOutputLocation = fileReaderProps.getPropValue(currentPrefix + CROP_SUFFIX_EXTRACTOR_FILE_OUTPUT);
            String intermediateFilesLocation = fileReaderProps.getPropValue(currentPrefix + CROP_SUFFIX_INTERMEDIATE_FILE_LOCATION);
            String isActiveString = fileReaderProps.getPropValue(currentPrefix + CROP_SUFFIX_INTERMEDIATE_FILE_ACTIVE);
            ...
            for (GobiiDbType currentDbType : GobiiDbType.values()) {
                ...
                String currentHost = fileReaderProps.getPropValue(currentDbPrefix + DB_SUFFIX_HOST);
                String currentDbName = fileReaderProps.getPropValue(currentDbPrefix + DB_SUFFIX_DBNAME);
                Integer currentPort = Integer.parseInt(fileReaderProps.getPropValue(currentDbPrefix + DB_SUFFIX_PORT));
                String currentUserName = fileReaderProps.getPropValue(currentDbPrefix + DB_SUFFIX_USER);
                String currentPassword = fileReaderProps.getPropValue(currentDbPrefix + DB_SUFFIX_PASSWORD);
