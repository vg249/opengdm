...
public class ListStatementLinkageGroupByMapBrapi implements ListStatement {
    ...
    public PreparedStatement makePreparedStatement(Connection dbConnection,
        ...
        if (sqlParamVals != null) {
            ...
            if(sqlParamVals.containsKey("mapId")) {

                mapId = (Integer) sqlParamVals.getOrDefault("mapId", 0);

                if(mapId < 1) {
                    throw new GobiiDaoException(GobiiStatusLevel.ERROR,
                            GobiiValidationStatusType.UNKNOWN, "Invalid Map Id");
                }
            }
            else {
                throw new GobiiDaoException(GobiiStatusLevel.ERROR,
        ...
        String sql = "WITH linkage_groups_paged AS " +
                "(SELECT * FROM linkage_group WHERE map_id = ? "+ pageSizeCondition+ ") " +
