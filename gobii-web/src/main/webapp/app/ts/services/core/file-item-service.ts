    ...
    public getForFilter(filterParamName: FilterParamNames): Observable<GobiiFileItem[]> {
        ...
        switch (filterParamName) {
            ...
            case FilterParamNames.MARKER_GROUPS:
                returnVal = this.store.select(fromRoot.getMarkerGroups) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.PROJECTS:
                returnVal = this.store.select(fromRoot.getProjects) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.PROJECTS_BY_CONTACT:
                returnVal = this.store.select(fromRoot.getProjectsByPI) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.EXPERIMENTS_BY_PROJECT:
                returnVal = this.store.select(fromRoot.getExperimentsByProject) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.EXPERIMENTS:
                returnVal = this.store.select(fromRoot.getExperiments) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.DATASETS_BY_EXPERIMENT:
                returnVal = this.store.select(fromRoot.getDatasetsByExperiment) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.PLATFORMS:
                returnVal = this.store.select(fromRoot.getPlatforms) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.CV_DATATYPE:
                returnVal = this.store.select(fromRoot.getCvTermsDataType) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.CV_JOB_STATUS:
                returnVal = this.store.select(fromRoot.getCvTermsJobStatus) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.MAPSETS:
                returnVal = this.store.select(fromRoot.getMapsets) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.CONTACT_PI_HIERARCHY_ROOT:
                returnVal = this.store.select(fromRoot.getPiContacts) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.CONTACT_PI_FILTER_OPTIONAL:
                returnVal = this.store.select(fromRoot.getPiContactsFilterOptional) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.PROJECT_FILTER_OPTIONAL:
                returnVal = this.store.select(fromRoot.getProjectsFilterOptional) as Observable<GobiiFileItem[]>;
                break;

            case FilterParamNames.EXPERIMENT_FILTER_OPTIONAL:
                returnVal = this.store.select(fromRoot.getExperimentsFilterOptional) as Observable<GobiiFileItem[]>;
                break;

            default:
                returnVal = this.store.select(fromRoot.getAllFileItems) as Observable<GobiiFileItem[]>;
                break;
                                ...
                                if (recurse) {
                                    if (filterParamsToLoad
                                        ...
                                        this.store.select(fromRoot.getAllFileItems)
                                            .subscribe((allFileItems: GobiiFileItem[]) => {
