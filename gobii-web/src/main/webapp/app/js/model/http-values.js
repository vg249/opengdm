System.register(["@angular/common/http", "./header-names"], function (exports_1, context_1) {
    ...
    return {
        ...
        execute: function () {
            HttpValues = class HttpValues {
                static makeTokenHeaders(token, gobiiCropType) {
                ...
                static makeContentHeaders() {
                    let returnVal = new http_1.HttpHeaders();
                    returnVal.append('Content-Type', 'application/json');
                    returnVal.append('Accept', 'application/json');
                    return returnVal;
                }
                static makeLoginHeaders(userName, password) {
                    let returnVal = this.makeContentHeaders();
