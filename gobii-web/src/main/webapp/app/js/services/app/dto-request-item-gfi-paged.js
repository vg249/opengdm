System.register(["@angular/core", "../../model/type-process", "../../model/filter-params", "../../model/filter-type", "../../model/file-item-param-names", "../../model/payload/paged-item-list", "../../model/payload/pagination"], function (exports_1, context_1) {
    ...
    return {
        ...
        execute: function () {
            DtoRequestItemGfiPaged = class DtoRequestItemGfiPaged {
                constructor(fileItemParams, id = null, jsonToGfi) {
                ...
                resultFromJson(json) {
                    let fileItems = [];
                    let currentPage = json.header.pagination.currentPage;
                    json.payload.data.forEach(jsonItem => {
                        let currentFileItem = this.jsonToGfi.convert(jsonItem);
                        currentFileItem.setPageNumber(currentPage);
                        fileItems.push(currentFileItem);
                    });
                    let returnVal = new paged_item_list_1.PagedFileItemList(fileItems, pagination_1.Pagination.fromJSON(json.header.pagination));
                    return returnVal;
                }
