System.register(["@angular/core", "../../model/type-entity", "../../views/entity-labels", "../../model/type-extractor-item", "../../model/type-extractor-filter", "../../model/cv-filter-type", "../../model/gobii-file-item", "../../model/dto-header-status-message", "../../model/type-process", "./name-id-service", "../../store/actions/history-action", "../../store/actions/fileitem-action", "../../store/reducers", "@ngrx/store", "../../model/name-id-label-type", "../../model/filter-type", "../../model/file-item-param-names", "rxjs", "rxjs/add/operator/expand", "rxjs/add/operator/concat", "./dto-request.service", "../app/dto-request-item-entity-stats", "./filter-params-coll", "../../model/gobii-file-item-entity-relation", "../../model/type-status-level", "../../store/actions/action-payload-filter"], function (exports_1, context_1) {
    ...
    return {
        ...
        execute: function () {
            FileItemService = class FileItemService {
                constructor(nameIdService, entityStatsService, fileItemRequestService, store, filterParamsColl) {
                ...
                loadFilter(gobiiExtractFilterType, filterParamsName, filterValue) {
                    let filterParams = this.filterParamsColl.getFilter(filterParamsName, gobiiExtractFilterType);
                    if (filterParams) {
                        let loadAction = new fileItemActions.LoadFilterAction({
                ...
                setItemLabelType(gobiiExtractFilterType, filterParamNames, nameIdLabelType) {
                    let nameIdRequestParamsFromType = this.filterParamsColl.getFilter(filterParamNames, gobiiExtractFilterType);
                ...
                loadNameIdsFromFilterParams(gobiiExtractFilterType, filterParamName, filterValue) {
                    let nameIdRequestParamsFromType = this.filterParamsColl.getFilter(filterParamName, gobiiExtractFilterType);
                    if (nameIdRequestParamsFromType) {
                ...
                loadFileItem(gobiiFileItem, selectForExtract) {
                    let loadAction = new fileItemActions.LoadFileItemtAction({
                 ...
                 */
                replaceFileItemByCompoundId(gobiiFileItem) {
                    let loadAction = new fileItemActions.ReplaceItemOfSameCompoundIdAction({
                        gobiiFileitemToReplaceWith: gobiiFileItem
                    });
                    this.store.dispatch(loadAction);
                }
                unloadFileItemFromExtract(gobiiFileItem) {
                    let loadAction = new fileItemActions.RemoveFromExtractAction(gobiiFileItem);
                    this.store.dispatch(loadAction);
                }
                /***
                 ...
                 */
                makeFileActionsFromFilterParamName(gobiiExtractFilterType, filterParamName, newTargetFilterValue) {
                    return rxjs_1.Observable.create(observer => {
                        let parentFilterParams = this.filterParamsColl.getFilter(filterParamName, gobiiExtractFilterType);
                        this.store
                            .select(fromRoot.getFileItemsFilters)
                            .subscribe(filters => {
                            let relatedEntityFiltervalueFromState = null;
                            let stateFilter = filters[filterParamName.toString()];
                            if (stateFilter) {
                            ...
                            if (parentFilterParams) {
                                ...
                                let parentLoadAction = new fileItemActions.LoadFilterAction({
                ...
                makeChildActions(gobiiExtractFilterType, filterParamName, parentFilterValue) {
                    let returnVal;
                    let parentFilterParams = this.filterParamsColl.getFilter(filterParamName, gobiiExtractFilterType);
                    if (parentFilterParams) {
                        // Now we process child filters
                        let filterParamsToProcess = null;
                ...
                makeNameIdLoadActions(gobiiExtractFilterType, filterParamName, filterValue) {
                    let nameIdRequestParamsFromType = this.filterParamsColl.getFilter(filterParamName, gobiiExtractFilterType);
                    if (nameIdRequestParamsFromType) {
                 ...
                 * @returns {Observable<LoadFileItemListWithFilterAction>}
                 */
                makeFileItemActionsFromNameIds(gobiiExtractFilterType, filterParamsToLoad, filterValue, recurse) {
                    return rxjs_1.Observable.create(observer => {
                         ...
                         */
                        this.entityStatsService.get(new dto_request_item_entity_stats_1.DtoRequestItemEntityStats(dto_request_item_entity_stats_1.EntityRequestType.LasetUpdated, filterParamsToLoad.getEntityType(), null, null))
                            .subscribe(entityStats => {
                            this.store.select(fromRoot.getFiltersRetrieved)
                                .subscribe(filterHistoryItems => {
                                let fileHistoryItem = filterHistoryItems.find(fhi => fhi.gobiiExtractFilterType === filterParamsToLoad.getGobiiExtractFilterType()
                                ...
                                if (disregardDateSensitiveQueryingForNow ||
                                    ...
                                    this.nameIdService.get(filterParamsToLoad, filterValue)
                                        .subscribe(nameIds => {
                                        let minEntityLastUpdated;
                                        let fileItems = [];
                                        if (nameIds && (nameIds.length > 0)) {
                                            nameIds.forEach(nameIdItem => {
                                                let entityRelation = null;
                                                ...
                                                if (nameIdItem.fkEntityType && nameIdItem.fkId) {
                                                    let gobiiFileItemCompoundUniqueId = null;
                                                ...
                                                let currentFileItem = gobii_file_item_1.GobiiFileItem.build(gobiiExtractFilterType, type_process_1.ProcessType.CREATE)
                                            ...
                                            if (filterParamsToLoad.getMameIdLabelType() != name_id_label_type_1.NameIdLabelType.UNKNOWN) {
                                                ...
                                                let labelFileItem = gobii_file_item_1.GobiiFileItem
                                        ...
                                        if (filterParamsToLoad.getQueryName() !== file_item_param_names_1.FilterParamNames.MARKER_GROUPS) {
                                            let noneFileItem = gobii_file_item_1.GobiiFileItem
                                        ...
                                        let parentId = fileItems[0].getItemId();
                                        //filterParamsToLoad.setTargetEntityFilterValue(parentId && +parentId > 0 ? parentId : null);
                                        let targetEntityFilterValue = parentId && +parentId > 0 ? parentId : null;
                                        let loadAction = new fileItemActions.LoadFileItemListWithFilterAction({
                                        ...
                                        if (recurse) {
                                            if (filterParamsToLoad
                                                ...
                                                .filter(rqp => rqp.getFilterType() === filter_type_1.FilterType.NAMES_BY_TYPEID)
                                                .length > 0) {
                                                for (let idx = 0; idx < filterParamsToLoad.getChildFileItemParams().length; idx++) {
                                                    let rqp = filterParamsToLoad.getChildFileItemParams()[idx];
                                                    if (rqp.getFilterType() === filter_type_1.FilterType.NAMES_BY_TYPEID) {
                                                        //rqp.setRelatedEntityFilterValue(parentId);
                                                        this.makeFileItemActionsFromNameIds(gobiiExtractFilterType, rqp, parentId, true)
                                    ...
                                    if (recurse) {
                                        if (filterParamsToLoad
                                            ...
                                            this.store.select(fromRoot.getAllFileItems)
                                                ...
                                                // already.
                                                let candidateParentFileItems = allFileItems.filter(fi => filterParamsToLoad.getTargetEtityUniqueId().compoundIdeEquals(fi)
                                                    && fi.getParentItemId() === filterValue);
                                                let childItemsFilterValue = null;
                                                ...
                                                for (let idx = 0; idx < filterParamsToLoad.getChildFileItemParams().length; idx++) {
                                                    ...
                                                    if (rqp.getFilterType() === filter_type_1.FilterType.NAMES_BY_TYPEID) {
                                                        ...
                                                        this.makeFileItemActionsFromNameIds(gobiiExtractFilterType, rqp, childItemsFilterValue, true)
                                                            .subscribe(fileItems => observer.next(fileItems));
                                                    }
                                                }
                ...
                recurseFilters(gobiiExtractFilterType, filterParamsToLoad, parentFilterValue, recurse) {
                    return rxjs_1.Observable.create(observer => {
                        ...
                        if (recurse) {
                            if (filterParamsToLoad
                                ...
                                this.store.select(fromRoot.getAllFileItems)
                                    ...
                                    let parentEntityCompoundUniqueId = this.filterParamsColl.getFilter(filterParamsToLoad.getParentFileItemParams().getQueryName(), gobiiExtractFilterType).getTargetEtityUniqueId();
                                    ...
                                    for (let idx = 0; idx < filterParamsToLoad.getChildFileItemParams().length; idx++) {
                                        ...
                                        this.recurseFilters(gobiiExtractFilterType, rqp, childItemsFilterValue, true)
                                            .subscribe(fileItems => observer.next(fileItems));
                                        //                                }
                                    }
                                }).unsubscribe(); //select all file items
                ...
                loadEntityList(gobiiExtractFilterType, fileItemParamName) {
                    let fileItemParams = this.filterParamsColl.getFilter(fileItemParamName, gobiiExtractFilterType);
                ...
                loadPagedEntityList(gobiiExtractFilterType, fileItemParamName, paedQueryId, pageSize, pageNum) {
                    let fileItemParams = this.filterParamsColl.getFilter(fileItemParamName, gobiiExtractFilterType);
                ...
                makeFileItemActionsFromEntities(gobiiExtractFilterType, filterParams, filterValue, recurse) {
                    return rxjs_1.Observable.create(observer => {
                        try {
                            ...
                            if (filterParams.getFilterType() === filter_type_1.FilterType.ENTITY_LIST) {
                                let dtoRequestItem = filterParams.getDtoRequestItem();
                                let dtoRequestService = filterParams.getDtoRequestService();
                                dtoRequestService
                                    .get(dtoRequestItem)
                                ...
                                let extractFilterTypeString = "undefined";
