System.register(["@angular/core", "@angular/router", "@ngrx/effects", "rxjs/add/operator/switchMap", "rxjs/add/observable/of", "rxjs/add/operator/concat", "../actions/fileitem-action", "../actions/treenode-action", "../../services/core/tree-structure-service", "../reducers", "../../store/actions/history-action", "../../model/type-extractor-item", "rxjs", "@ngrx/store", "../../services/core/file-item-service", "../../model/file-item-param-names", "rxjs/add/operator/mergeMap", "../../services/core/filter-params-coll", "rxjs/operators"], function (exports_1, context_1) {
    ...
    return {
        ...
        execute: function () {
            FileItemEffects = class FileItemEffects {
                // @Effect()
                ...
                //
                //     }); //switch map
                constructor(actions$, treeStructureService, fileItemService, store, filterParamsColl, router) {
                    ...
                    this.selectForExtract$ = this.actions$
                        .pipe(effects_1.ofType(fileItemActions.ADD_TO_EXTRACT), operators_1.map((action) => {
                        let treeNode = this.treeStructureService.makeTreeNodeFromFileItem(action.payload);
                        return new treeNodeActions.PlaceTreeNodeAction(treeNode);
                    }));
                    this.replaceSameByCompoundId$ = this.actions$
                        .pipe(effects_1.ofType(fileItemActions.REPLACE_ITEM_OF_SAME_COMPOUND_ID), operators_1.map((action) => {
                        let treeNode = this.treeStructureService.makeTreeNodeFromFileItem(action.payload.gobiiFileitemToReplaceWith);
                        return new treeNodeActions.PlaceTreeNodeAction(treeNode);
                    }));
                    this.loadFileItemListWithFilter$ = this.actions$
                        .pipe(effects_1.ofType(fileItemActions.LOAD_FILE_ITEM_LIST_WITH_FILTER), operators_1.switchMap((action) => {
                        return rxjs_1.Observable.create(observer => {
                            let addFilterSubmittedAction = new historyAction
                                .AddFilterRetrieved({
                            ...
                            let filterParams = this.filterParamsColl.getFilter(action.payload.filterId, action.payload.filter.gobiiExtractFilterType);
                            let gobiiFileItems = action.payload.gobiiFileItems;
                            let payloadFilter = action.payload.filter;
                    ...
                    this.replaceInExtract$ = this.actions$
                        .pipe(effects_1.ofType(fileItemActions.REPLACE_BY_ITEM_ID), operators_1.switchMap((action) => {
                        ...
                        return rxjs_1.Observable.create(observer => {
                            let fileItemToReplaceWithUniqueId = action.payload.itemIdToReplaceItWith;
                            let fileItemCurrentlyInExtractUniqueId = action.payload.itemIdCurrentlyInExtract;
                            let filterParamName = action.payload.filterParamName;
                            this.store.select(fromRoot.getAllFileItems)
                                .subscribe((all) => {
                                let fileItemToReplaceWith = all.find(fi => fi.getFileItemUniqueId() === fileItemToReplaceWithUniqueId);
                                ...
                                let filterValue = (fileItemToReplaceWith.getItemId() && Number(fileItemToReplaceWith.getItemId()) > 0) ? fileItemToReplaceWith.getItemId() : null;
                                if (filterParamName !== file_item_param_names_1.FilterParamNames.UNKNOWN) {
                                    this.fileItemService.makeFileActionsFromFilterParamName(action.payload.gobiiExtractFilterType, filterParamName, filterValue).subscribe(loadFileItemListAction => {
                                ...
                                if (fileItemToReplaceWith.getIsExtractCriterion()) {
                                    if (fileItemToReplaceWith.getExtractorItemType() != type_extractor_item_1.ExtractorItemType.LABEL) {
                                        let treeNode = this.treeStructureService.makeTreeNodeFromFileItem(fileItemToReplaceWith);
                    ...
                    this.selectForExtractByFileItemId$ = this.actions$
                        .pipe(effects_1.ofType(fileItemActions.ADD_TO_EXTRACT_BY_ITEM_ID), operators_1.switchMap((action) => {
                        ...
                        return rxjs_1.Observable.create(observer => {
                            let fileItemUniqueId = action.payload;
                            this.store.select(fromRoot.getAllFileItems)
                                .subscribe((all) => {
                                let fileItem = all.find(fi => fi.getFileItemUniqueId() === fileItemUniqueId);
                                let treeNode = this.treeStructureService.makeTreeNodeFromFileItem(fileItem);
                    ...
                    this.deSelectFromExtract$ = this.actions$
                        .pipe(effects_1.ofType(fileItemActions.REMOVE_FROM_EXTRACT), operators_1.map((action) => {
                    ...
                    this.deSelectFromExtractById$ = this.actions$
                        .pipe(effects_1.ofType(fileItemActions.REMOVE_FROM_EXTRACT_BY_ITEM_ID), operators_1.map((action) => {
                        return new treeNodeActions.DeActivateFromExtractAction(action.payload);
                    }));
                    this.deselectAll$ = this.actions$
                        .pipe(effects_1.ofType(fileItemActions.REMOVE_ALL_FROM_EXTRACT), operators_1.map((action) => {
                        return new treeNodeActions.ClearAll(action.payload);
                    }));
                    this.loadFileItem = this.actions$
                        .pipe(effects_1.ofType(fileItemActions.LOAD_FILE_ITEM), operators_1.map((action) => {
                    ...
                    this.setExtractType = this.actions$
                        .pipe(effects_1.ofType(fileItemActions.SET_EXTRACT_TYPE), operators_1.map((action) => {
